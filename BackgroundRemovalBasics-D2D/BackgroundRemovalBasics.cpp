...
int CBackgroundRemovalBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	UINT dataLength = m_colorWidth * m_colorHeight * cBytesPerPixel;

	for (UINT l = 0; l < dataLength; ++l)
		m_backgroundRGBX[l] = UCHAR_MAX;

	for (UINT c = 0; c < m_colorHeight; ++c)
	{
		for (UINT r = 0; r < m_colorWidth; ++r)
		{
			//x * Height * Depth + y * Depth + z;
			UINT index = c * m_colorWidth * cBytesPerPixel + r * cBytesPerPixel;
			m_backgroundRGBX[index] = 0; //setting the R channel;
			m_backgroundRGBX[index + 2] = 1 / UCHAR_MAX; //setting B channel
